...
export default async function decorate(block) {
  ...
  logoLink.href = '/';
  logoLink.innerHTML = `<?xml version="1.0" encoding="UTF-8"?>
<svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 302">
  ...
  // Insert logo at the beginning of the footer
  const firstChild = footer.firstElementChild;
  if (firstChild) {
    footer.insertBefore(footerLogo, firstChild);
  } else {
    footer.appendChild(footerLogo);
  }

  // Remove images from default-content-wrapper before appending
  fragment.querySelectorAll('.default-content-wrapper img').forEach(img => img.remove());
